...
int main()
{
  ...
  int fd = open("/dev/ttyUSB1",O_RDWR | O_NOCTTY | O_NDELAY);	/* ttyUSB0 is the FT232 based USB2SERIAL Converter   */
  ...
  Mat imgcrop2 = cvimg(roi2);
    ...
    circle(resultado,Point(xd,yd),10,255);
    imshow("Camera", resultado);
	}
  ...
  xi[0] = posX1;
  xi[1] = posY1;
  xi[2] = revol*2*PI + atan2((float)posY2-(float)posY1, (float)posX2-(float)posX1);
	if(xia[2]-xi[2] > PI)
		revol++;
	if(xia[2]-xi[2] < -PI)
		revol--;
  ...
  yt = yd-xi[1];
  tht = thd-xi[2];
  float zeta1 = kp1*tanh(xt) - kd1*xip[0];
	float zeta2 = kp2*tanh(yt) - kd2*xip[1];
  float zeta3 = kp3*tanh(tht) - kd3*xip[2];

	u[0] = zeta1+zeta2+zeta3;
	u[1] = zeta1-zeta2-zeta3;
	u[2] = zeta1+zeta2-zeta3;
	u[3] = zeta1-zeta2+zeta3;
...
else
{
  ...
  if(nFeature==3)
    ...
    //printf("%f%f%f%f\r\n",u[0],u[1],u[2],u[3]);
    //printf("%c%c%c - %d%c%d%c%d%c%d%c\r\n",write_buffer[0],write_buffer[1],write_buffer[2],write_buffer[3],write_buffer[4],write_buffer[5],write_buffer[6],write_buffer[7],write_buffer[8],write_buffer[9],write_buffer[10]);
		//fprintf(datos,"%5.3f,%5.3f,%5.3f,%5.3f,%5.3f,%6.3f,%6.3f,%6.3f,%4.3f,%4.3f,%4.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f,%6.3f;\r\n",t1,u[0],u[1],u[2],u[3],xi[0],xi[1],xi[2],xip[0],xip[1],xip[2],xd,yd,thd,xt,yt,tht);
	}
}
	//fprintf(datos,"];");
	fclose(datos);
